...
import { getMatchingCustomerIds, lastServicedByCustomer, laterDate, realServiceDate, type MarketingFilters } from '@/lib/marketing/query'
import { cleanContactName } from '@/lib/names'
...
export async function fetchContactsForIds(db: SupabaseClient<any>, customerIds: string[]): Promise<MailchimpContact[]> {
  ...
  for (const c of (contactsData ?? []) as RawContact[]) {
    ...
    const { first, last } = cleanContactName(c.first_name, c.last_name)
    contactMap.set(c.customer_id, { first_name: first, last_name: last, email, phone })
  }
  ...
  for (const c of (customers ?? []) as RawCustomer[]) {
    ...
    contacts.push({
      ...
      last_serviced_date: laterDate(lastServiceMap.get(c.id) ?? null, realServiceDate(c.last_serviced_date)),
...
export async function POST(req: NextRequest) {
  ...
  await db.from('mailchimp_push_log').insert({
    ...
    skipped_count: result.audience_skipped,
    failed_count: result.audience_errored,
    contact_results: {
      audience_unchanged: result.audience_unchanged,
      tagged: result.tagged,
      not_taggable: result.not_taggable,
      sms_set: result.sms_set,
      sms_skipped: result.sms_skipped,
