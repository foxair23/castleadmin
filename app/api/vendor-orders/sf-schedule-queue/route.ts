...
import { getSfScheduleQueue } from '@/lib/vendor-orders/sf-schedule-queue'

export const dynamic = 'force-dynamic'

// GET — Genie appointments the browser extension should write onto existing SF jobs.
//
// Service Fusion's API cannot modify a job that already exists (PUT /jobs/{id} → 405), so
// this mirrors the remittance and line-item queues: the app decides what to write, the
// extension writes it through SF's logged-in web session. Same shared-token guard and open
// CORS as those routes; public in proxy.ts under /api/vendor-orders/.
...
export async function GET(req: NextRequest) {
  ...
  const { items } = await getSfScheduleQueue(limit)
